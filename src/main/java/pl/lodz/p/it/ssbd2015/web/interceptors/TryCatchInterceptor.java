...
public class TryCatchInterceptor {
    ...
    public Object tryCatch(InvocationContext ctx) throws Exception
    {
        ...
        {
            logger.error("Error occurred while editing PersonEntity: " + ctx.getParameters().toString());
            e.printStackTrace();

            ((BaseContextBean)ctx.getTarget()).setContext(ApplicationErrorBean.class, bean -> {
                bean.setExceptionMessage(e.getCode());
            });
